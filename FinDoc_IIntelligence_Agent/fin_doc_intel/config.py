"""
Configuration management for FinDoc Intelligence Agent.
...
class Config:
    ...
    # Fundamentals pipeline model tiers (relevance/classification vs. extraction).
    # Default to llm_model_name unless overridden — single-model setups need no change.
    small_llm_model_name: str = "anthropic.claude-4-5-haiku"
    large_llm_model_name: str = "anthropic.claude-sonnet-4-6"
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            ...
            small_llm_model_name=os.getenv("SMALL_LLM_MODEL_NAME", default_model),
            large_llm_model_name=os.getenv("LARGE_LLM_MODEL_NAME", default_model),
...
def get_llm(model_name: Optional[str] = None, max_tokens: Optional[int] = None) -> ChatOpenAI:
    """Return a chat LLM client pointed at the configured gateway.

    `model_name`/`max_tokens` override the configured defaults — used by the
    fundamentals extraction pipeline to run cheap classification nodes on the
    small model tier and the line-item extraction node with a larger token budget.
    """
    config = get_config()
    return ChatOpenAI(
